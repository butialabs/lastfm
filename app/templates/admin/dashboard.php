...
<div class="card mb-4">
	<div class="card-body">
		<form method="get" action="/admin" class="row g-3">
			<div class="col-md-2">
				<label for="filter_protocol"
					class="form-label"><?= htmlspecialchars(__('admin.filter.protocol'), ENT_QUOTES) ?></label>
				<select class="form-select" id="filter_protocol" name="protocol">
					...
					<option value="at" <?= ($filters['protocol'] ?? '') === 'at' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.bluesky'), ENT_QUOTES) ?></option>
					<option value="mastodon" <?= ($filters['protocol'] ?? '') === 'mastodon' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.mastodon'), ENT_QUOTES) ?></option>
				</select>
			</div>
			<div class="col-md-2">
				<label for="filter_status"
					class="form-label"><?= htmlspecialchars(__('admin.filter.status'), ENT_QUOTES) ?></label>
				<select class="form-select" id="filter_status" name="status">
					<option value=""><?= htmlspecialchars(__('admin.filter.all'), ENT_QUOTES) ?></option>
					<option value="ACTIVE" <?= ($filters['status'] ?? '') === 'ACTIVE' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.status.active'), ENT_QUOTES) ?></option>
					<option value="SCHEDULE" <?= ($filters['status'] ?? '') === 'SCHEDULE' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.status.schedule'), ENT_QUOTES) ?></option>
					<option value="QUEUED" <?= ($filters['status'] ?? '') === 'QUEUED' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.status.queued'), ENT_QUOTES) ?></option>
					<option value="SENDING" <?= ($filters['status'] ?? '') === 'SENDING' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.status.sending'), ENT_QUOTES) ?></option>
					<option value="ERROR" <?= ($filters['status'] ?? '') === 'ERROR' ? 'selected' : '' ?>>
						<?= htmlspecialchars(__('admin.status.error'), ENT_QUOTES) ?></option>
				</select>
			</div>
			<div class="col-md-3">
				<label for="filter_search"
					class="form-label"><?= htmlspecialchars(__('admin.filter.search'), ENT_QUOTES) ?></label>
				<input type="text" class="form-control" id="filter_search" name="search"
					value="<?= htmlspecialchars($filters['search'] ?? '', ENT_QUOTES) ?>"
					placeholder="<?= htmlspecialchars(__('admin.filter.search_placeholder'), ENT_QUOTES) ?>">
			</div>
			<div class="col-md-2">
				<label for="filter_language"
					class="form-label"><?= htmlspecialchars(__('admin.filter.language'), ENT_QUOTES) ?></label>
				<select class="form-select" id="filter_language" name="language">
					<option value=""><?= htmlspecialchars(__('admin.filter.all'), ENT_QUOTES) ?></option>
					<option value="en" <?= ($filters['language'] ?? '') === 'en' ? 'selected' : '' ?>>
			...
			<div class="col-md-1">
				<label for="filter_limit"
					class="form-label"><?= htmlspecialchars(__('admin.filter.per_page'), ENT_QUOTES) ?></label>
			...
			<div class="col-md-2 d-flex align-items-end gap-2">
				<button type="submit"
					class="btn btn-primary"><?= htmlspecialchars(__('admin.filter.button'), ENT_QUOTES) ?></button>
				<a href="/admin"
					class="btn btn-outline-secondary"><?= htmlspecialchars(__('admin.filter.clear'), ENT_QUOTES) ?></a>
			</div>
		</form>
	</div>
</div>
				...
				<?php if (empty($users)): ?>
					<tr>
						<td colspan="11" class="text-center text-muted py-4">
							<?= htmlspecialchars(__('admin.table.no_users'), ENT_QUOTES) ?></td>
					</tr>
				<?php else: ?>
					<?php foreach ($users as $user): ?>
						<tr>
							...
							<td class="text-center">
								...
								?>
								<a href="<?= $instanceUrl ?>" target="_blank" rel="noopener"
									title="<?= $protocolTitle ?>: <?= htmlspecialchars($user['instance'] ?? '', ENT_QUOTES) ?>">
									<i class="bi <?= $iconClass ?>" style="font-size: 1.2rem;"></i>
								</a>
							</td>
							<td class="text-truncate" style="max-width: 150px;"
								title="<?= htmlspecialchars($user['username'] ?? '', ENT_QUOTES) ?>">
								<?php
								$profileUrl = null;
								if ($user['protocol'] === 'at') {
								...
								?>
								<?php if ($profileUrl): ?>
									<a href="<?= htmlspecialchars($profileUrl, ENT_QUOTES) ?>" target="_blank" rel="noopener"
										title="<?= htmlspecialchars(__('admin.table.view_profile'), ENT_QUOTES) ?>">
							...
							<td>
								<?php if (!empty($user['lastfm_username'])): ?>
									<a href="https://last.fm/user/<?= htmlspecialchars($user['lastfm_username'], ENT_QUOTES) ?>"
							...
							<td>
								...
								?>
								<span
									class="badge <?= $statusClass ?>"><?= htmlspecialchars($user['status'] ?? '', ENT_QUOTES) ?></span>
							</td>
							<td>
							...
							<td class="text-truncate" style="max-width: 100px;"
								title="<?= htmlspecialchars($user['timezone'] ?? '', ENT_QUOTES) ?>">
								<?= htmlspecialchars($user['timezone'] ?? '-', ENT_QUOTES) ?>
							</td>
							<td>
								<?php if ((int) $user['error_count'] > 0): ?>
									<span class="badge bg-danger"
										title="<?= htmlspecialchars($user['callback'] ?? '', ENT_QUOTES) ?>">
							...
							<td>
								<?= htmlspecialchars(substr($user['created_at'] ?? '', 0, 10), ENT_QUOTES) ?> /
								<?= htmlspecialchars(substr($user['updated_at'] ?? '', 0, 10), ENT_QUOTES) ?>
							</td>
							<td class="text-center">
		...
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="userDetailsModalLabel">
					<?= htmlspecialchars(__('admin.modal.user_details'), ENT_QUOTES) ?></h5>
			...
			<div class="modal-body" id="userDetailsModalBody">
				<div class="text-center py-4">
					<div class="spinner-border text-primary" role="status">
						<span
							class="visually-hidden"><?= htmlspecialchars(__('admin.modal.loading'), ENT_QUOTES) ?></span>
					</div>
